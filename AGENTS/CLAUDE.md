...
dp-web4 remotes are SSH (`git@github.com:auraecosystem/web4.0.git`). SSH key already loaded by ssh-agent — just `git push` / `git pull`. Do NOT use HTTPS+PAT (the `GITHUB_PAT` env var is deprecated and fails).
